//#region src/util/uniqueID.ts
var e = 1, t = () => e++, n = (...e) => e.reduce((e, t) => e > t ? e : t), r = (...e) => e.reduce((e, t) => e < t ? e : t), i = (...e) => {
	let t = n(...e);
	...
	let i = document.createElement("canvas"), a = t || e.clientWidth || window.innerWidth, o = n || e.clientHeight || window.innerHeight;
	return i.width = a * r, i.height = o * r, i.style.border = "0px", i.style.margin = "0px", i.style.padding = "0px", i.style.top = "0px", i.style.left = "0px", i.style.width = `${a}px`, i.style.height = `${o}px`, e === document.body && (i.style.position = "absolute"), e.appendChild(i), i;
}, c = (e) => {
	let t = document.body, n;
	if (e.selector instanceof HTMLCanvasElement) n = e.selector;
	else if (typeof e.selector == "string" && e.selector.constructor == String) n = document.getElementById(e.selector);
	else throw Error("[E][parseContextDesc] opts.selector invalid.");
	let r = e.width * devicePixelRatio, i = e.height * devicePixelRatio;
	return n ? (n.width = r, n.height = i, n.style.border = "0px", n.style.margin = "0px", n.style.padding = "0px", n.style.top = "0px", n.style.left = "0px", n.style.width = `${e.width}px`, n.style.height = `${e.height}px`) : n = s(t, e.width, e.height, e.devicePixelRatio), {
		selector: e.selector,
		width: e.width,
		height: e.height,
		viewportWidth: r,
		viewportHeight: i,
		devicePixelRatio: e.devicePixelRatio || devicePixelRatio || 1,
		container: t,
		canvas: n
	};
}, l = class {
	...
	limits;
	frameTargetTexture;
	frameTargetTextureView;
	commandEncoder;
	supportedFeatures;
	requestFeatures;
	constructor(e) {
		this.contextDesc = c(e), this.gpuContext = this.contextDesc.canvas.getContext("webgpu"), this.requestFeatures = e.requestFeatures;
	}
	async init() {
		if (this.adapter = await navigator.gpu.requestAdapter(), !this.adapter) throw Error("[E][Context][init] get adapter failed.");
		this.supportedFeatures = this.adapter.features, (this.requestFeatures || []).forEach((e) => {
			if (!this.supportedFeatures.has(e)) throw Error(`[E][Context][init] init context failed. unsupported feature: ${e}`);
		...
		}), this.limits = this.adapter?.limits, this.features = this.adapter?.features, this.queue = this.device?.queue;
	}
	get GPUDescription() {
		return this.adapter?.info.description || "unknown";
	}
	getSupportedFeatures = () => this.features;
	refreshFrameResource = () => {
		this.frameTargetTexture = this.gpuContext?.getCurrentTexture(), this.frameTargetTextureView = this.frameTargetTexture.createView(), this.commandEncoder = this.device?.createCommandEncoder();
	};
	submitFrameResource = () => {
		let e = [this.commandEncoder?.finish({})];
		this.queue?.submit(e);
	};
	getCommandEncoder = () => this.commandEncoder;
	getFrameTexture = () => this.frameTargetTexture;
	getFrameTextureView = () => this.frameTargetTextureView;
	getViewportWidth = () => this.contextDesc.viewportWidth;
	getViewportHeight = () => this.contextDesc.viewportHeight;
	getMax2DMipmapCount = () => i(this.getViewportWidth(), this.getViewportHeight());
	getGpuDevice = () => this.device;
	getGpuQueue = () => this.queue;
	getLimits = () => this.limits;
	getPreferredTextureFormat = () => navigator.gpu.getPreferredCanvasFormat();
	getPreferredDepthTexuteFormat = () => "depth24plus";
}, u = {
	rewrite: !1,
	details: []
}, ee = {
	...
	debugLabel;
	constructor(e) {
		this.id = e.id, this.context = e.context, this.poropertyFormat = e.poropertyFormat, this.debugLabel = e.debugLabel;
	}
	getID = () => this.id;
	...
	constructor(e) {
		...
		}), this.computePipeline = e.computePipeline, this.bufferState = e.bufferState, this.textureState = e.textureState, this.computeHandler = e.computeHandler, this.uniformHandler = e.uniformHandler, this.hookHandler = e.hookHandler, this.slotBindGroupMap = e.slotBindGroupMap;
	}
	build(e) {
		this.uniformHandler("frameBegin", e, this.bufferState, this.textureState);
		let t = e.beginComputePass({ label: "compute holder" });
		t.setPipeline(this.computePipeline.getGpuComputePipeline()), this.slotBindGroupMap.forEach((e, n) => {
			t.setBindGroup(n, e);
	...
	constructor(e) {
		...
		}), this.renderPipeline = e.renderPipeline, this.bufferState = e.bufferState, this.texturteState = e.texturteState, this.renderHandler = e.renderHandler, this.uniformHandler = e.uniformHandler, this.slotAttributeBufferIDMap = e.slotAttributeBufferIDMap, this.slotBindGroupMap = e.slotBindGroupMap, this.colorAttachments = e.colorAttachments, this.depthStencilAttachment = e.depthStencilAttachment;
	}
	build = (e) => {
		if (this.uniformHandler("frameBegin", e, this.bufferState, this.texturteState), this.colorAttachments.length === 0) {
			console.log("[E][RenderHolder][build] missing color attachment.");
			return;
		}
		let t = [];
		this.colorAttachments.forEach((e) => {
			t.push(e.getGpuColorAttachment());
		});
		let n = { colorAttachments: t };
		this.depthStencilAttachment && (n.depthStencilAttachment = this.depthStencilAttachment.getGpuRenderPassDepthStencilAttachment());
		let r = e.beginRenderPass(n);
		r.setPipeline(this.renderPipeline.getGpuRenderPipeline()), this.slotAttributeBufferIDMap.forEach((t, n) => {
			r.setVertexBuffer(n, this.bufferState.getBuffer(t)?.getGpuBuffer(e, "frameBegin"));
		}), this.slotBindGroupMap.forEach((e, t) => {
			r.setBindGroup(t, e);
	...
	buffer;
	totalByteLength = 0;
	latestTotalByteLength = 0;
	label;
	constructor(e) {
		if (this.id = e.id, this.context = e.context, this.label = e.label, this.bufferUsageFlags = e.bufferUsageFlags, this.totalByteLength = this.latestTotalByteLength = e.totalByteLength, !this.totalByteLength) throw Error("[E][BaseBuffer][constructor] create buffer error, opts.totalByteLength value invalid.");
	}
	getID = () => this.id;
	getByteLength = () => this.totalByteLength;
	expand(e) {
	...
	constructor(e) {
		if (super({
			...
			label: e.label,
			context: e.context,
			bufferUsageFlags: e.bufferUsageFlags,
			totalByteLength: e.totalByteLength
		}), this.handler = e.handler, this.typedArrayData1D = e.rawData, !this.handler && !this.typedArrayData1D) throw Error("[E][Buffer1D][constructor] create buffer error, either opts.handler or opts.typedArrayData1D must be assigned a value.");
	}
	updateGpuBuffer = (e, t, n) => {
		if (e + t > this.totalByteLength || n.byteLength > this.totalByteLength) throw Error(`[E][VertexBuffer][updateGpuBuffer] buffer bytelength oversized, maximum bytelength: ${this.totalByteLength}`);
		try {
			this.context?.getGpuQueue().writeBuffer(this.buffer, e, n, 0);
		} catch (e) {
			console.error(`[updateGpuBuffer] ${this.label} error. message: ${e} `);
		}
	};
	createGpuBuffer = () => {
		if (!this.buffer) {
			...
			this.buffer = this.context.getGpuDevice().createBuffer(e);
		}
	...
	constructor(e) {
		if (super({
			...
			rawData: e.typedArrayData1D
		}), this.typedArrayData1D instanceof Uint16Array) this.indexFormat = "uint16", this.drawCount = this.typedArrayData1D.byteLength / this.typedArrayData1D.BYTES_PER_ELEMENT;
		else if (this.typedArrayData1D instanceof Uint32Array) this.indexFormat = "uint32", this.drawCount = this.typedArrayData1D.byteLength / this.typedArrayData1D.BYTES_PER_ELEMENT;
		else throw Error("[E][IndexBuffer][constructor] index data type error.");
	}
	...
	constructor(e) {
		super({
			...
			context: e.context,
			totalByteLength: e.totalByteLength,
			bufferUsageFlags: e.bufferUsageFlags | GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST
		}), this.rawDataArray = e.rawData, this.handler = e.handler;
	}
	updateGpuBuffer = (e, t, n) => {
		if (e + t > this.totalByteLength || n.byteLength > this.totalByteLength) throw Error(`[E][VertexBuffer][updateGpuBuffer] buffer bytelength oversized, maximum bytelength: ${this.totalByteLength}`);
		try {
	...
	createGpuBuffer = () => {
		...
		if (this.rawDataArray) {
			let e = 0;
			this.rawDataArray?.forEach((t) => {
				this.updateGpuBuffer(e, t.byteLength, t), e += t.byteLength;
			});
		} else if (this.handler) {
			let e = this.handler();
	...
	constructor(e) {
		super({
			...
			context: e.context,
			totalByteLength: e.totalByteLength,
			bufferUsageFlags: e.bufferUsageFlags || GPUBufferUsage.STORAGE | GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
			rawData: e.rawDataArray,
			handler: e.handler
		});
	}
}, p = class extends f {
	constructor(e) {
		if (super({
			...
			rawDataArray: e.rawDataArray,
			handler: e.handler
		}), this.totalByteLength % 20 != 0) throw Error(`[E][IndirectBuffer] indexed indirect buffer align byte length is 20, please cheack buffer total byte length. current total byte length: ${this.totalByteLength}`);
	}
	getStride = () => 20;
	getOffset = () => 0;
	...
	constructor(e) {
		if (super({
			...
			let e = "none";
			this.rawDataArray.forEach((t) => {
				if (t instanceof Uint16Array && (e === "uint16" || e === "none")) e = "uint16", this.drawCount += t.length;
				else if (t instanceof Uint32Array && (e === "uint32" || e === "none")) e = "uint32", this.drawCount += t.length;
				else throw Error("[E][IndexedStorageBuffer][constructor] raw data error. only one type of uint16/uint32 support. and all slice of raw must be the same type.");
			}), this.indexedFormat = e;
		}
	}
	getIndexedFormat = () => this.indexedFormat;
	getMaxDrawCount = () => {
		let e = this.indexedFormat === "uint16" ? Uint16Array.BYTES_PER_ELEMENT : Uint32Array.BYTES_PER_ELEMENT;
	...
	constructor(e) {
		if (super({
			...
			rawDataArray: e.rawDataArray,
			handler: e.handler
		}), this.totalByteLength % 16 != 0) throw Error(`[E][IndirectBuffer] indirect buffer align byte length is 16, please cheack buffer total byte length. current total byte length: ${this.totalByteLength}`);
	}
	getStride = () => 16;
	getOffset = () => 0;
	getIndexIndirectCount = () => this.rawDataArray?.length || 0;
	...
	constructor(e) {
		super({
			...
			label: e.label,
			context: e.context,
			bufferUsageFlags: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST | GPUBufferUsage.COPY_SRC | (e.appendixBufferUsageFlags || 0),
			totalByteLength: e.totalByteLength,
			rawDataArray: e.rawDataArray,
			handler: e.handler
		});
	...
	createMapReadBuffer = () => {
		...
		this.mapReadBuffer = this.context?.getGpuDevice().createBuffer(e);
	};
	createMapWriteBuffer = () => {
		let e = {
			size: this.getByteLength(),
			label: `[MapBuffer][createMapWriteBuffer][${this.label}]`,
			usage: GPUBufferUsage.MAP_WRITE | GPUBufferUsage.COPY_SRC
		};
		this.mapWriteBuffer = this.context?.getGpuDevice().createBuffer(e);
	};
	async PushDataAsync(e, t, n) {
		this.buffer || this.createGpuBuffer(), this.mapWriteBuffer || this.createMapWriteBuffer();
		let r = this.getByteLength();
		if (e + t > r) {
			console.log(`[E][MapBuffer][PushDataAsync] write buffer leak/out of memory: ${e + t}.`);
			return;
		}
		await this.mapWriteBuffer.mapAsync(GPUMapMode.WRITE, 0, r), new Float32Array(this.mapWriteBuffer.getMappedRange()).set(n, e), this.mapWriteBuffer.unmap();
	}
	async PullDataAsync(e, t) {
		this.buffer || this.createGpuBuffer(), this.mapReadBuffer || this.createMapReadBuffer();
		let n = this.getByteLength(), r = e || 0, i = t || 0;
		if (r + i > n) throw Error(`[E][MapBuffer][PullDataAsync] pull gpu-side buffer failed. byte length oversize: ${r + i}.`);
		await this.mapReadBuffer.mapAsync(GPUMapMode.READ, 0, n);
		let a = this.mapReadBuffer.getMappedRange().slice(0);
		return this.mapReadBuffer.unmap(), a;
	}
	getGpuBuffer = (e, t) => {
		if (super.getGpuBuffer(e, t), this.mapReadBuffer || this.createMapReadBuffer(), this.mapWriteBuffer || this.createMapWriteBuffer(), this.buffer || this.createGpuBuffer(), t === "frameFinish" && e) {
			let t = this.getByteLength();
			e.copyBufferToBuffer(this.buffer, this.mapReadBuffer, t);
		}
		return this.buffer;
	};
}, ce = class extends ie {
	...
	getBuffer = (t) => {
		if (e.BUFFER_SET.has(t)) return e.BUFFER_SET.get(t);
	...
	createIndexBuffer(n) {
		let r = t(), i = new ae({
			...
			totalByteLength: n.rawData.byteLength,
			typedArrayData1D: n.rawData
		});
	...
	createUniformBuffer = (n) => {
		let r = t(), i = new ce({
			...
			label: n.label,
			context: this.context,
			totalByteLength: n.totalByteLength,
			rawData: n.rawData,
			handler: n.handler
		});
		return e.BUFFER_SET.set(r, i), e.BUFFER_SET.get(r);
	...
	createMapBuffer = (n) => {
		let r = t(), i = new se({
			...
			context: this.context,
			totalByteLength: n.totalByteLength,
			appendixBufferUsageFlags: n.appendixBufferUsageFlags,
			rawDataArray: n.rawDataArray,
			handler: n.handler
		});
		return e.BUFFER_SET.set(r, i), e.BUFFER_SET.get(r);
	};
	createStorageBuffer = (n) => {
		let r = t(), i = new f({
			...
			totalByteLength: n.totalByteLength,
			rawDataArray: n.rawDataArray,
			bufferUsageFlags: n.bufferUsageFlags,
			handler: n.handler
		});
		return e.BUFFER_SET.set(r, i), e.BUFFER_SET.get(r);
	};
	...
	reflectedUniforms;
	constructor(t) {
		this.id = e.hash32aID(t.code, t.entryPoint), this.context = t.context, this.shaderStage = t.shaderStage, this.code = t.code, this.entryPoint = t.entryPoint;
	}
	static hash32aID = (e, t) => a(`${e}-${t}`);
	getID = () => this.id;
	getEntryPoint = () => this.entryPoint;
	getGpuShader = () => this.shader;
	createGpuShader = (e) => {
		if (!this.shader) {
			let t = {
				label: e,
			...
			this.shader = this.context?.getGpuDevice().createShaderModule(t), this.shader?.getCompilationInfo().then((t) => {
				t.messages.forEach((t) => {
					throw Error(`[E][BaseShader][createGpuShader] label:${e}, error message: ${t}.`);
				});
			});
		}
	};
...
function C(e, t, n, r) {
	...
	for (let a = 0; a < r; ++a) switch (n) {
		...
		case "32uint":
		case "32sint":
			i[a] = e[t] | e[t + 1] << 8 | e[t + 2] << 16 | e[t + 3] << 24, t += 4;
			break;
		case "32float": i[a] = new Float32Array(e.buffer, t, 1)[0], t += 4;
	}
	return i;
}
function w(e, t, n, r, i) {
	for (let a = 0; a < r; ++a) switch (n) {
		...
		case "8sint":
			...
			break;
		case "16uint":
			new Uint16Array(e.buffer, t, 1)[0] = i[a], t += 2;
			break;
		case "16sint":
		...
		case "32uint":
			new Uint32Array(e.buffer, t, 1)[0] = i[a], t += 4;
			break;
		case "32sint":
			new Int32Array(e.buffer, t, 1)[0] = i[a], t += 4;
			break;
		case "32float": new Float32Array(e.buffer, t, 1)[0] = i[a], t += 4;
	}
	return i;
}
var ke = {
...
var Me = new Set(/* @__PURE__ */ "all.all.any.select.arrayLength.abs.acos.acosh.asin.asinh.atan.atanh.atan2.ceil.clamp.cos.cosh.countLeadingZeros.countOneBits.countTrailingZeros.cross.degrees.determinant.distance.dot.dot4U8Packed.dot4I8Packed.exp.exp2.extractBits.faceForward.firstLeadingBit.firstTrailingBit.floor.fma.fract.frexp.insertBits.inverseSqrt.ldexp.length.log.log2.max.min.mix.modf.normalize.pow.quantizeToF16.radians.reflect.refract.reverseBits.round.saturate.sign.sin.sinh.smoothStep.sqrt.step.tan.tanh.transpose.trunc.dpdx.dpdxCoarse.dpdxFine.dpdy.dpdyCoarse.dpdyFine.fwidth.fwidthCoarse.fwidthFine.textureDimensions.textureGather.textureGatherCompare.textureLoad.textureNumLayers.textureNumLevels.textureNumSamples.textureSample.textureSampleBias.textureSampleCompare.textureSampleCompareLevel.textureSampleGrad.textureSampleLevel.textureSampleBaseClampToEdge.textureStore.atomicLoad.atomicStore.atomicAdd.atomicSub.atomicMax.atomicMin.atomicAnd.atomicOr.atomicXor.atomicExchange.atomicCompareExchangeWeak.pack4x8snorm.pack4x8unorm.pack4xI8.pack4xU8.pack4x8Clamp.pack4xU8Clamp.pack2x16snorm.pack2x16unorm.pack2x16float.unpack4x8snorm.unpack4x8unorm.unpack4xI8.unpack4xU8.unpack2x16snorm.unpack2x16unorm.unpack2x16float.storageBarrier.textureBarrier.workgroupBarrier.workgroupUniformLoad.subgroupAdd.subgroupExclusiveAdd.subgroupInclusiveAdd.subgroupAll.subgroupAnd.subgroupAny.subgroupBallot.subgroupBroadcast.subgroupBroadcastFirst.subgroupElect.subgroupMax.subgroupMin.subgroupMul.subgroupExclusiveMul.subgroupInclusiveMul.subgroupOr.subgroupShuffle.subgroupShuffleDown.subgroupShuffleUp.subgroupShuffleXor.subgroupXor.quadBroadcast.quadSwapDiagonal.quadSwapX.quadSwapY".split(".")), E = class extends T {
	...
	search(e) {
		if (this.attributes) for (let t of this.attributes) e(t);
		e(this);
...
var We = class extends E {
	...
	static maxFormatType(t) {
		let n = t[0];
		if (n.name === "f32") return n;
		for (let r = 1; r < t.length; ++r) {
			let i = e._priority.get(n.name);
			e._priority.get(t[r].name) < i && (n = t[r]);
		}
		return n.name === "x32" ? e.i32 : n;
	}
	getTypeName() {
		return this.name;
	}
...
var it = class extends A {
	...
	search(e) {
		if (e(this), this.args) for (let t of this.args) t.search(e);
	}
	constEvaluate(e, t) {
...
var G = class e extends H {
	constructor(e, t, n = null) {
		if (super(t, n), e instanceof Float32Array || e instanceof Uint32Array || e instanceof Int32Array) this.data = e;
			...
			t === "vec2f" || t === "vec3f" || t === "vec4f" ? this.data = new Float32Array(e) : t === "vec2i" || t === "vec3i" || t === "vec4i" ? this.data = new Int32Array(e) : t === "vec2u" || t === "vec3u" || t === "vec4u" ? this.data = new Uint32Array(e) : t === "vec2h" || t === "vec3h" || t === "vec4h" ? this.data = new Float32Array(e) : t === "vec2b" || t === "vec3b" || t === "vec4b" ? this.data = new Int32Array(e) : t === "vec2" || t === "vec3" || t === "vec4" ? this.data = new Float32Array(e) : console.error(`VectorData: Invalid type ${t}`);
		}
	}
	clone() {
		if (this.data instanceof Float32Array) return new e(new Float32Array(this.data), this.typeInfo, null);
			...
			if (n instanceof z) {
				...
				if (!(n instanceof F)) return console.error("GetDataValue: Unknown postfix type", n), null;
				{
					...
					if (a instanceof v) {
						...
						if (n === "vec2f" || n === "vec3f" || n === "vec4f" || n === "vec2i" || n === "vec3i" || n === "vec4i" || n === "vec2u" || n === "vec3u" || n === "vec4u" || n === "vec2b" || n === "vec3b" || n === "vec4b" || n === "vec2h" || n === "vec3h" || n === "vec4h" || n === "vec2" || n === "vec3" || n === "vec4") {
							if (e.length > 0 && e.length < 5) {
								...
								for (let a = 0; a < e.length; ++a) {
									let s = e[a].toLowerCase(), c = 0;
									if (s === "x" || s === "r") c = 0;
									else if (s === "y" || s === "g") c = 1;
									else if (s === "z" || s === "b") c = 2;
									else {
										if (s !== "w" && s !== "a") return console.error(`Unknown member ${e}`), null;
										c = 3;
									}
									if (e.length === 1) {
									...
									if (n === "vec2f") o.push(new Float32Array(this.buffer, i, 2)[c]);
										...
										let e = new Float32Array(this.buffer, i, 3);
										o.push(e[c]);
									} else if (n === "vec4f") o.push(new Float32Array(this.buffer, i, 4)[c]);
									else if (n === "vec2i") r = "i", o.push(new Int32Array(this.buffer, i, 2)[c]);
									else if (n === "vec3i") r = "i", o.push(new Int32Array(this.buffer, i, 3)[c]);
									else if (n === "vec4i") r = "i", o.push(new Int32Array(this.buffer, i, 4)[c]);
									else if (n === "vec2u") {
										r = "u";
										let e = new Uint32Array(this.buffer, i, 2);
										o.push(e[c]);
						...
						return console.error(`GetDataValue: Type ${n} is not a struct`), null;
					}
				}
			}
			n = n.postfix;
	...
	get width() {
		var e;
		let t = this.descriptor.size;
		return t instanceof Array && t.length > 0 ? t[0] ?? 0 : t instanceof Object && (e = t.width) != null ? e : 0;
	}
	get height() {
		var e;
		let t = this.descriptor.size;
		return t instanceof Array && t.length > 1 ? t[1] ?? 0 : t instanceof Object && (e = t.height) != null ? e : 0;
	}
	get depthOrArrayLayers() {
		var e;
		let t = this.descriptor.size;
		return t instanceof Array && t.length > 2 ? t[2] ?? 0 : t instanceof Object && (e = t.depthOrArrayLayers) != null ? e : 0;
	}
	get format() {
		var e;
		return this.descriptor && (e = this.descriptor.format) != null ? e : "rgba8unorm";
	}
	get sampleCount() {
		var e;
		return this.descriptor && (e = this.descriptor.sampleCount) != null ? e : 1;
	}
	get mipLevelCount() {
		var e;
		return this.descriptor && (e = this.descriptor.mipLevelCount) != null ? e : 1;
	}
	get dimension() {
		var e;
		return this.descriptor && (e = this.descriptor.dimension) != null ? e : "2d";
	}
	getMipLevelSize(e) {
	...
	setPixel(e, t, n, r, i) {
		...
		})(new Uint8Array(c), e, t, n, r, s, o, a, this.format, i);
	}
};
((e) => {
			...
			if (this._peekAhead() == "*") {
				...
				for (; t > 0;) {
					...
					if (e = this._advance(), e == "\n") this._line++;
						...
						if (this._peekAhead() == "/" && (this._advance(), t--, t == 0)) return !0;
...
var Mt = class {
	...
	getFunctionInfo(e) {
		for (let t of this.functions) if (t.name == e) return t;
		return null;
	}
	getStructInfo(e) {
	...
	getOverrideInfo(e) {
		for (let t of this.overrides) if (t.name == e) return t;
		return null;
	}
	_markStructsInUse(e) {
		if (e) if (e.isStruct) {
	...
	_addCalls(e, t) {
		for (let n of e.calls) {
			let e = this._functions.get(n.name)?.info;
			e && t.add(e);
		}
	}
	findResource(e, t, n) {
		if (n) {
			for (let r of this.entry.compute) if (r.name === n) {
				for (let n of r.resources) if (n.group == e && n.binding == t) return n;
			}
			for (let r of this.entry.vertex) if (r.name === n) {
				for (let n of r.resources) if (n.group == e && n.binding == t) return n;
			}
			for (let r of this.entry.fragment) if (r.name === n) {
				for (let n of r.resources) if (n.group == e && n.binding == t) return n;
			}
		}
		for (let n of this.uniforms) if (n.group == e && n.binding == t) return n;
		for (let n of this.storage) if (n.group == e && n.binding == t) return n;
		for (let n of this.textures) if (n.group == e && n.binding == t) return n;
		for (let n of this.samplers) if (n.group == e && n.binding == t) return n;
		return null;
	}
	_findResource(e) {
		for (let t of this.uniforms) if (t.name == e) return t;
		for (let t of this.storage) if (t.name == e) return t;
		for (let t of this.textures) if (t.name == e) return t;
		for (let t of this.samplers) if (t.name == e) return t;
		return null;
	}
	_markStructsFromAST(e) {
		let t = this.getTypeInfo(e, null);
	...
	getBindGroups() {
		let e = [];
		function t(t, n) {
			t >= e.length && (e.length = t + 1), e[t] === void 0 && (e[t] = []), n >= e[t].length && (e[t].length = n + 1);
		}
		for (let n of this.uniforms) t(n.group, n.binding), e[n.group][n.binding] = n;
		for (let n of this.storage) t(n.group, n.binding), e[n.group][n.binding] = n;
		for (let n of this.textures) t(n.group, n.binding), e[n.group][n.binding] = n;
		for (let n of this.samplers) t(n.group, n.binding), e[n.group][n.binding] = n;
		return e;
	}
	...
	_getAlias(e) {
		for (let t of this.aliases) if (t.name == e) return t.type;
		return null;
	}
	_getAliasInfo(e) {
	...
	_updateStructInfo(e) {
		let t = 0, n = 0, r = 0, i = 0;
		for (let a = 0, o = e.members.length; a < o; ++a) {
			let o = e.members[a], s = this._getTypeSize(o);
			if (!s) continue;
			this._getAlias(o.type.name) ?? o.type;
			let c = s.align, l = s.size;
			t = this._roundUp(c, t + n), n = l, r = t, i = Math.max(i, c), o.offset = t, o.size = l, this._updateTypeInfo(o.type);
		}
		e.size = this._roundUp(i, r + n), e.align = i;
	}
	_getTypeSize(t) {
	...
	_getAttribute(e, t) {
		...
		let r = n.attributes;
		for (let e of r) if (e.name == t) return e;
		return null;
	}
	_getAttributeNum(e, t, n) {
		if (e === null) return n;
		for (let r of e) if (r.name == t) {
			let e = r !== null && r.value !== null ? r.value : n;
			return e instanceof Array && (e = e[0]), typeof e == "number" ? e : typeof e == "string" ? parseInt(e) : n;
		}
		return n;
	}
	_roundUp(e, t) {
...
var Ft = 0, It = class e {
	...
	setVariable(e, t, n) {
		...
		r === null ? this.createVariable(e, t, n) : r.value = t;
	}
	getVariableValue(e) {
		return this.getVariable(e)?.value ?? null;
	}
	clone() {
		return new e(this);
	}
	...
	Refract(e, t) {
		...
		if (n instanceof G && r instanceof G && i instanceof W) {
			...
			return new G(n.data.map((t, n) => {
				...
				if (a < 0) return 0;
				let o = Math.sqrt(a);
				return i.value * t - (i.value * e + o) * r.data[n];
			}), n.typeInfo);
		}
	...
	AtomicLoad(e, t) {
		let n = e.args[0];
		n instanceof B && (n = n.right);
		let r = this.exec.getVariableName(n, t);
	...
	execute(e) {
		(e ??= {}).constants && this._setOverrides(e.constants, this.context), this._execStatements(this.ast, this.context);
	}
	dispatchWorkgroups(e, t, n, r) {
		let i = this.context.clone();
		(r ??= {}).constants && this._setOverrides(r.constants, i), this._execStatements(this.ast, i);
		let a = i.getFunction(e);
	...
	_dispatchExec(e, t) {
		for (let n of e.node.args) for (let e of n.attributes) if (e.name === "builtin") {
			let r = `@${e.value}`, i = t.getVariable(r);
			i !== void 0 && t.variables.set(n.name, i);
		}
	...
	_call(e, t) {
		...
		let r = t.getFunction(e.name);
		if (r) {
			for (let t = 0; t < r.node.args.length; ++t) {
				let i = r.node.args[t], a = this.evalExpression(e.args[t], n);
				n.setVariable(i.name, a, i);
			}
	...
	_assign(e, t) {
		...
		if (e.variable instanceof B) {
			...
			i = e.variable.postfix, r = this.getVariableName(e.variable, t);
			let a = t.getVariable(r);
			...
			if (i instanceof z) {
					...
					if (!(n instanceof K)) return void console.error(`Invalid assignment to ${r}. Line ${e.line}`);
					{
						...
						if (o < 0 || !(a instanceof G)) return void console.error(`Invalid assignment to ${r}. Line ${e.line}`);
						{
							...
							if (t === "mat2x2" || t === "mat2x2f" || t === "mat2x2h") {
								if (!(o < 2 && a.data.length === 2)) return void console.error(`Invalid assignment to ${r}. Line ${e.line}`);
								n.data[2 * o] = a.data[0], n.data[2 * o + 1] = a.data[1];
							} else if (t === "mat2x3" || t === "mat2x3f" || t === "mat2x3h") {
								if (!(o < 2 && a.data.length === 3)) return void console.error(`Invalid assignment to ${r}. Line ${e.line}`);
								n.data[3 * o] = a.data[0], n.data[3 * o + 1] = a.data[1], n.data[3 * o + 2] = a.data[2];
							} else if (t === "mat2x4" || t === "mat2x4f" || t === "mat2x4h") {
								if (!(o < 2 && a.data.length === 4)) return void console.error(`Invalid assignment to ${r}. Line ${e.line}`);
								n.data[4 * o] = a.data[0], n.data[4 * o + 1] = a.data[1], n.data[4 * o + 2] = a.data[2], n.data[4 * o + 3] = a.data[3];
							} else if (t === "mat3x2" || t === "mat3x2f" || t === "mat3x2h") {
								if (!(o < 3 && a.data.length === 2)) return void console.error(`Invalid assignment to ${r}. Line ${e.line}`);
								n.data[2 * o] = a.data[0], n.data[2 * o + 1] = a.data[1];
							} else if (t === "mat3x3" || t === "mat3x3f" || t === "mat3x3h") {
								if (!(o < 3 && a.data.length === 3)) return void console.error(`Invalid assignment to ${r}. Line ${e.line}`);
								n.data[3 * o] = a.data[0], n.data[3 * o + 1] = a.data[1], n.data[3 * o + 2] = a.data[2];
							} else if (t === "mat3x4" || t === "mat3x4f" || t === "mat3x4h") {
								if (!(o < 3 && a.data.length === 4)) return void console.error(`Invalid assignment to ${r}. Line ${e.line}`);
								n.data[4 * o] = a.data[0], n.data[4 * o + 1] = a.data[1], n.data[4 * o + 2] = a.data[2], n.data[4 * o + 3] = a.data[3];
							} else if (t === "mat4x2" || t === "mat4x2f" || t === "mat4x2h") {
								if (!(o < 4 && a.data.length === 2)) return void console.error(`Invalid assignment to ${r}. Line ${e.line}`);
								n.data[2 * o] = a.data[0], n.data[2 * o + 1] = a.data[1];
							} else if (t === "mat4x3" || t === "mat4x3f" || t === "mat4x3h") {
								if (!(o < 4 && a.data.length === 3)) return void console.error(`Invalid assignment to ${r}. Line ${e.line}`);
								n.data[3 * o] = a.data[0], n.data[3 * o + 1] = a.data[1], n.data[3 * o + 2] = a.data[2];
							} else {
								if (t !== "mat4x4" && t !== "mat4x4f" && t !== "mat4x4h" || !(o < 4 && a.data.length === 4)) return void console.error(`Invalid assignment to ${r}. Line ${e.line}`);
								n.data[4 * o] = a.data[0], n.data[4 * o + 1] = a.data[1], n.data[4 * o + 2] = a.data[2], n.data[4 * o + 3] = a.data[3];
							}
						}
					}
				...
				if (a instanceof W) {
					...
					if (t === "x") n.data[0] = a.value;
						...
						if (n.data.length < 2) return void console.error(`Invalid assignment to ${t} for variable ${r}. Line ${e.line}`);
						n.data[1] = a.value;
					} else if (t === "z") {
						if (n.data.length < 3) return void console.error(`Invalid assignment to ${t} for variable ${r}. Line ${e.line}`);
						n.data[2] = a.value;
					} else if (t === "w") {
						if (n.data.length < 4) return void console.error(`Invalid assignment to ${t} for variable ${r}. Line ${e.line}`);
						n.data[3] = a.value;
					}
				} else {
					...
					for (let i = 0; i < t.length; ++i) {
						let o = t[i];
						if (o === "x" || o === "r") n.data[0] = a.data[i];
						else if (o === "y" || o === "g") {
							if (a.data.length < 2) return void console.error(`Invalid assignment to ${o} for variable ${r}. Line ${e.line}`);
							n.data[1] = a.data[i];
						} else if (o === "z" || o === "b") {
							if (a.data.length < 3) return void console.error(`Invalid assignment to ${o} for variable ${r}. Line ${e.line}`);
							n.data[2] = a.data[i];
						} else {
							if (o !== "w" && o !== "a" || a.data.length < 4) return void console.error(`Invalid assignment to ${o} for variable ${r}. Line ${e.line}`);
							n.data[3] = a.data[i];
						}
					}
				}
	...
	_for(t, n) {
		for (n = n.clone(), this.execStatement(t.init, n); this._getScalarValue(this.evalExpression(t.condition, n));) {
			let r = this._execStatements(t.body, n);
			if (r === e._breakObj) break;
			if (r !== null && r !== e._continueObj) return r;
			this.execStatement(t.increment, n);
		}
		return null;
	}
	_loop(t, n) {
		for (n = n.clone();;) {
			...
			if (r === e._breakObj) break;
			if (r === e._continueObj) {
				if (t.continuing && this._execStatements(t.continuing.body, n) === e._breakObj) break;
			} else if (r !== null) return r;
		}
		return null;
	}
	_while(t, n) {
		for (n = n.clone(); this._getScalarValue(this.evalExpression(t.condition, n));) {
			let r = this._execStatements(t.body, n);
			if (r === e._breakObj) break;
			if (r !== e._continueObj && r !== null) return r;
		}
		return null;
	}
	_evalBitcast(e, t) {
		...
		if (n instanceof G) {
			...
			else if (t.endsWith("u")) i = "u32";
			else if (t.endsWith("b")) i = "bool";
			else {
				if (!t.endsWith("h")) return console.error(`Unknown vector type ${t}. Line ${e.line}`), null;
				i = "f16";
			}
			let a = r.getTypeName(), o = "";
			if (a.endsWith("f")) o = "f32";
			else if (a.endsWith("i")) o = "i32";
			else if (a.endsWith("u")) o = "u32";
			else if (a.endsWith("b")) o = "bool";
			else {
				...
				if (Z(i) && Z(a)) {
					...
					if (n instanceof K && r instanceof K) {
						let i = function(e, t, n, r) {
							...
							for (let t = 0; t < a; t++) for (let r = 0; r < o; r++) {
								let c = 0;
								for (let o = 0; o < i; o++) c += e[o * a + t] * n[r * i + o];
								s[t * o + r] = c;
							}
							return s;
					...
					if (n instanceof K && r instanceof G) {
						let i = function(e, t, n, r) {
							...
							for (let t = 0; t < a; t++) {
								let r = 0;
								for (let o = 0; o < i; o++) r += e[o * a + t] * n[o];
								o[t] = r;
							}
							return o;
						}(t, n.typeInfo, o, r.typeInfo);
					...
					if (n instanceof G && r instanceof K) {
						let i = function(e, t, n, r) {
							...
							for (let t = 0; t < i; t++) {
								let r = 0;
								for (let o = 0; o < a; o++) r += e[o] * n[o * i + t];
								o[t] = r;
							}
							return o;
						}(t, n.typeInfo, o, r.typeInfo);
	...
	_evalCall(e, t) {
		...
		n.currentFunctionName = e.name;
		let r = t.getFunction(e.name);
		if (!r) return e.isBuiltin ? this._callBuiltinFunction(e, n) : this.getTypeInfo(e.name) ? this._evalCreate(e, t) : (console.error(`Unknown function "${e.name}". Line ${e.line}`), null);
		for (let t = 0; t < r.node.args.length; ++t) {
			let i = r.node.args[t], a = this.evalExpression(e.args[t], n);
			n.createVariable(i.name, a, i);
		}
	...
	_callBuiltinFunction(e, t) {
		...
		let n = t.getFunction(e.name);
		if (n) {
			let r = t.clone();
			for (let t = 0; t < n.node.args.length; ++t) {
				let i = n.node.args[t], a = this.evalExpression(e.args[t], r);
				r.setVariable(i.name, a, i);
			}
...
var Ut = class {
	...
	parse(e) {
		...
		let t = [];
		for (; !this._isAtEnd();) {
			let e = this._global_decl_or_directive();
			if (!e) break;
			t.push(e);
		}
	...
	_advance() {
		return this._currentLine = this._peek()?.line ?? -1, this._isAtEnd() || this._current++, this._previous();
	}
	_peek() {
	...
	_singular_expression() {
		let e = this._primary_expression(), t = this._postfix_expression();
	...
	switch (e.type.name) {
		...
		case "texture_depth_multisampled_2d": return "2d";
		case "texture_2d_array":
		case "texture_storage_2d_array":
		case "texture_depth_2d_array": return "2d-array";
		case "texture_cube":
		case "texture_depth_cube": return "cube";
		case "texture_cube_array":
		case "texture_depth_cube_array": return "cube-array";
		case "texture_3d":
	...
	switch (e) {
		...
		case "depth24plus-stencil8":
		case "depth32float":
		case "depth32float-stencil8": return "depth";
		case "r8uint":
		case "rg8uint":
		case "rgba8uint":
		case "r16uint":
	...
	switch (n) {
		...
		case GPUShaderStage.FRAGMENT:
			a.entry.fragment.forEach((e) => {
				e.name === t && (o = e);
			});
			break;
		case GPUShaderStage.COMPUTE:
			a.entry.compute.forEach((e) => {
				e.name === t && (o = e);
			});
			break;
		default: throw Error(`[E][reflectShaderUniforms] ${i} unsupported shader stage ${n}`);
	}
	if (o === void 0) throw Error(`[E][reflectShaderUniforms] ${i} entry point "${t}" not found in the shader code.`);
	let s = /* @__PURE__ */ new Map(), c = /* @__PURE__ */ new Map(), l = (e) => (s.has(e) || s.set(e, []), s.get(e)), u = (e) => (c.has(e) || c.set(e, []), c.get(e));
	return o.resources.forEach((e) => {
		let t = {
			binding: e.binding,
			visibility: n
		...
		switch (e.resourceType) {
			...
			default: throw Error(`[E][reflectShaderUniforms] ${i} unsupported resource binding resource type: ${e.resourceType}`);
		}
	}), {
		bindGroupCount: s.size,
	...
	switch (e.getTypeName()) {
		...
		case "vec2u":
		case "vec2<u32>": return "uint32x2";
		case "vec3u":
		case "vec3<u32>": return "uint32x3";
		case "vec4u":
		case "vec4<u32>": return "uint32x4";
		case "i32": return "sint32";
		case "vec2i":
		case "vec2<i32>": return "sint32x2";
		case "vec3i":
		case "vec3<i32>": return "sint32x3";
		case "vec4i":
		case "vec4<i32>": return "sint32x4";
	...
	for (let e = 0; e < l; e++) {
		...
		s.set(t.name, n), o.set(n.shaderLocation, t.name), c.push(n);
	}
	return c.sort((e, t) => e.shaderLocation - t.shaderLocation), {
		attributeCount: l,
		attributeMap: s,
		attributeOdered: c,
		locationMap: o
	...
	switch (e) {
		...
		case "texture2DArray": return "2d-array";
		case "textureCube": return "cube";
		case "texutre3D": return "3d";
		case "textureCubeArray": return "cube-array";
	...
	switch (e) {
		case "texture1D": return "1d";
		case "texture2D": return "2d";
		case "texture2DArray": return "2d";
		case "textureCube": return "2d";
		case "texutre3D": return "3d";
		case "textureCubeArray": return "2d";
		case "textureStorage2D": return "2d";
	...
	switch (n) {
		...
		case "r8sint":
		case "stencil8":
			r.bytesPerRow = e, r.rowsPerImage = t;
			break;
		case "rg8unorm":
		case "rg8sint":
		case "r16float":
		...
		case "r16unorm":
		case "r16snorm":
			r.bytesPerRow = e * 2, r.rowsPerImage = t;
			break;
		case "rgba8unorm":
		case "rgba8snorm":
		case "r32uint":
		...
		case "rg16snorm":
		case "rg16unorm":
			r.bytesPerRow = e * 4, r.rowsPerImage = t;
			break;
		case "depth32float-stencil8":
			r.bytesPerRow = e * 5, r.rowsPerImage = t;
			break;
		case "rgba16uint":
		case "rgba16sint":
		case "eac-rg11unorm":
		...
		case "rgba16unorm":
		case "rgba16snorm":
			r.bytesPerRow = e * 8, r.rowsPerImage = t;
			break;
		case "rgba32float":
		case "rgba32uint":
		case "rgba32sint":
			r.bytesPerRow = e * 16, r.rowsPerImage = t;
			break;
		case "bc1-rgba-unorm":
		case "bc1-rgba-unorm-srgb":
		case "bc2-rgba-unorm":
		...
		case "etc2-rgba8unorm-srgb":
			r.bytesPerRow = e / 4 * 8, r.rowsPerImage = t / 4;
			break;
		case "bc6h-rgb-float":
		case "bc6h-rgb-ufloat":
		case "bc7-rgba-unorm":
		case "bc7-rgba-unorm-srgb":
			r.bytesPerRow = e / 4 * 16, r.rowsPerImage = t / 4;
			break;
		case "astc-4x4-unorm":
		case "astc-4x4-unorm-srgb":
		case "astc-10x10-unorm":
		...
		case "astc-12x10-unorm-srgb":
		case "astc-12x12-unorm":
		case "astc-12x12-unorm-srgb": throw Error("[E][getTexelCopyBufferLayout] unsupport astc format in PC platform.");
	...
	selectedUsage = "NONE";
	constructor(e) {
		if (this.id = e.id, this.context = e.context, this.width = e.width, this.height = e.height, this.width === 0 || this.height === 0) throw Error(`[E][Texture] texture parameter error, not support width or height equals 0, please check. texture id:${this.id}`);
	...
	cursor = (e) => {
		this.mipCurosr = e % this.mipmapCount;
	};
	...
	constructor(e) {
		super({
			id: e.id,
			context: e.context,
			width: e.context.getViewportWidth(),
			height: e.context.getViewportHeight(),
			textureUsageFlags: (e.appendixTextureUsages || 0) | GPUTextureUsage.COPY_DST | GPUTextureUsage.RENDER_ATTACHMENT | GPUTextureUsage.TEXTURE_BINDING,
			propertyFormat: "texture2D",
			textureFormat: e.context.getPreferredTextureFormat()
		});
	}
	createGpuTexture() {
		this.texture = this.context.getFrameTexture();
	}
	getGpuTextureView = () => this.context.getFrameTextureView();
	getGpuTexture = (e, t) => (this.createGpuTexture(), this.texture);
	useAsStorageBinding() {
	...
	getGpuTexture = (e, t) => (this.texture || this.createGpuTexture(), this.texture);
	getGpuTextureView = () => {
		switch (this.selectedUsage) {
			case "RENDER_ATTACHMENT": return this.renderAttachmentView;
			case "TEXTURE_BINDING": return this.textureBindingView;
			case "STORAGE_BINDING": {
				let e = this.storageBindingView[this.mipCurosr];
				return this.autoIncrementMipLevelInStorageBindingUse && this.nextCursor(), e;
			}
			default: throw Error("[E][Texture2D][getGpuTextureView] error occurs in get gpu texture view. for no 'useAs..' function called before compiler.");
		}
	};
	useAsStorageBinding = () => {
		if (!this.isUsageIncludeStorageBinding()) throw Error(`[E][Texture][useAsStorageBinding] ${typeof this} has no usage include 'GPUTextureUsage.STORAGE_BINDING', please check.`);
		if (this.selectedUsage = "STORAGE_BINDING", this.texture || this.createGpuTexture(), !this.storageBindingView) {
			...
			for (let e = 0; e < this.mipmapCount; e++) {
				...
				switch (t.baseArrayLayer = 0, t.arrayLayerCount = 1, t.baseMipLevel = e, t.mipLevelCount = 1, this.textureFormat) {
					...
					case "depth24plus":
					case "depth32float":
						t.aspect = "depth-only", t.mipLevelCount = 1;
						break;
					case "stencil8":
						t.aspect = "stencil-only", t.mipLevelCount = 1;
						break;
					case "depth24plus-stencil8":
					case "depth32float-stencil8":
	...
	useAsTextureBinding = () => {
		if (!this.isUsageIncludeTextureBinding()) throw Error(`[E][Texture][useAsTextureBinding] ${typeof this} has no usage include 'GPUTextureUsage.TEXTURE_BINDING', please check.`);
		if (this.selectedUsage = "TEXTURE_BINDING", this.texture || this.createGpuTexture(), !this.textureBindingView) {
			let e = {};
			switch (e.baseArrayLayer = 0, e.arrayLayerCount = 1, e.baseMipLevel = 0, e.mipLevelCount = this.mipmapCount, this.textureFormat) {
				case "depth16unorm":
				case "depth24plus":
				case "depth32float":
					e.aspect = "depth-only", e.mipLevelCount = 1;
					break;
				case "stencil8":
					e.aspect = "stencil-only", e.mipLevelCount = 1;
					break;
				case "depth24plus-stencil8":
				case "depth32float-stencil8":
	...
	useAsRenderAttachment = () => {
		if (!this.isUsageIncludeTextureBinding()) throw Error(`[E][Texture][useAsRenderAttachment] ${typeof this} has no usage include 'GPUTextureUsage.RENDER_ATTACHMENT', please check.`);
		if (this.selectedUsage = "RENDER_ATTACHMENT", this.texture || this.createGpuTexture(), !this.renderAttachmentView) {
			let e = {};
	...
	constructor(e) {
		...
		}), this.textureData2DArray = e.textureDataArray, this.handler = e.handler;
	}
	refreshTextureDataSource() {
		if (this.textureData2DArray && this.textureData2DArray.length > 0 && !this.isDetphTexture()) {
			let e = { texture: this.texture }, t = this.getTexelCopyBufferLayout(), n = {
				width: this.width,
				height: this.height,
				depthOrArrayLayers: 1
			};
			for (let r = 0, i = this.textureData2DArray.length; r < i; r++) e.origin = [
				0,
				0,
				r
			], this.context.getGpuQueue().writeTexture(e, this.textureData2DArray[r].buffer, t, n);
	...
	getGpuTexture(e, t) {
		return this.texture || this.createGpuTexture(), this.refreshTextureDataSource(), this.texture;
	}
	getGpuTextureView() {
		switch (this.selectedUsage) {
			case "RENDER_ATTACHMENT": return this.renderAttachmentView;
			...
			case "STORAGE_BINDING": {
				let e = this.storageBindingView[this.mipCurosr];
				return this.nextCursor(), e;
			}
			default: throw Error("[E][Texture2DArray][getGpuTextureView] error occurs in get gpu texture array view. for no 'useAs..' function called before compiler.");
		}
	}
	useAsStorageBinding = () => {
		if (!this.isUsageIncludeStorageBinding()) throw Error(`[E][Texture][useAsStorageBinding] ${typeof this} has no usage include 'GPUTextureUsage.STORAGE_BINDING', please check.`);
		if (this.selectedUsage = "STORAGE_BINDING", this.texture || this.createGpuTexture(), !this.storageBindingView) {
			...
			for (let e = 0; e < this.mipmapCount; e++) {
				let t = {};
				switch (t.baseArrayLayer = 0, t.arrayLayerCount = this.extent3d.depthOrArrayLayers, t.baseMipLevel = e, t.mipLevelCount = 1, this.textureFormat) {
					case "depth16unorm":
					case "depth24plus":
					case "depth32float":
						t.aspect = "depth-only", t.mipLevelCount = 1;
	...
	useAsTextureBinding = () => {
		...
		if (this.selectedUsage = "TEXTURE_BINDING", this.texture || this.createGpuTexture(), !this.textureBindingView) {
			let e = {};
			switch (e.baseArrayLayer = 0, e.arrayLayerCount = this.extent3d.depthOrArrayLayers, e.baseMipLevel = 0, e.mipLevelCount = this.mipmapCount, this.textureFormat) {
				case "depth16unorm":
				case "depth24plus":
				case "depth32float":
					e.aspect = "depth-only", e.mipLevelCount = 1;
	...
	getTexture = (t) => {
		if (!e.TEXTURE_SET.has(t)) throw Error(`[E][TextureState][getTexture] find texture failed, id: ${t}`);
	...
	createTexture2DArray = (n) => {
		let r = t(), i = new pn({
			...
			appendixTextureUsages: n.appendixTextureUsages,
			textureDataArray: n.textureDataArray,
			handler: n.handler,
			textureFormat: n.textureFormat,
			mipmapCount: n.mipmapCount
		});
	...
	return e.bufferAttributeRecordsMap.forEach((r, i) => {
		let a = 0;
		if (t?.forEach((t) => {
			let o = e.vertexShader.getAttributeNameByLocation(t.shaderLocation);
			if (!o) return;
			let s = r.get(o);
			if (!s) return;
			...
			e.vertexBufferLayouts.push(n);
		}
	}), t?.forEach((t) => {
		let n = e.vertexShader.getAttributeNameByLocation(t.shaderLocation);
		n && e.bufferAttributeRecordsMap.forEach((r, i) => {
			r.has(n) && e.slotBufferIDMap.set(t.shaderLocation, i);
		});
	}), {
		module: e.vertexShader.getGpuShader(),
	...
	let t = (t, n) => {
		if (!e.bufferAttributeRecordsMap.has(t)) {
			...
			e.bufferAttributeRecordsMap.set(t, n);
		}
		e.bufferAttributeRecordsMap.get(t)?.set(n.name, n);
	};
	(e.attributes?.getPropertyMap())?.forEach((n, r) => {
		let i = n.getPropertyFormat();
		switch (i) {
			case "vertexBuffer": {
				let a = n.getVertexBufferID(), o = {
					name: r,
					type: i,
					offset: 0,
					stride: 0,
				...
				t(a, o), e.attributeRecordMap.set(r, o);
				break;
			}
	...
	constructor(e) {
		super({
			id: e.id,
			context: e.context
		}), this.clearColor.r = e.clearColor ? e.clearColor[0] : this.clearColor.r, this.clearColor.g = e.clearColor ? e.clearColor[1] : this.clearColor.g, this.clearColor.b = e.clearColor ? e.clearColor[2] : this.clearColor.b, this.clearColor.a = e.clearColor ? e.clearColor[3] : this.clearColor.a, this.colorLoadStoreFormat = e.colorLoadStoreFormat ? e.colorLoadStoreFormat : "clearStore", this.blendFormat = e.blendFormat ? e.blendFormat : "opaque", this.texture = e.texture;
	}
	updateAttachment = () => {
		switch (this.texture.useAsRenderAttachment(), this.renderPassColorAttachment = {
			view: this.texture.getGpuTextureView(),
			loadOp: "clear",
			storeOp: "store",
			clearValue: this.clearColor
		}, this.colorLoadStoreFormat) {
			case "clearStore":
		...
		switch (this.blendFormat) {
			case "addAlphaSrcOneDst":
				this.blendState = {
					color: {},
					alpha: {}
				}, this.blendState.color.srcFactor = "src-alpha", this.blendState.color.dstFactor = "one-minus-src-alpha", this.blendState.color.operation = "add", this.blendState.alpha.srcFactor = "zero", this.blendState.alpha.dstFactor = "one", this.blendState.alpha.operation = "add";
				break;
			case "opaque":
				this.blendState = {
					color: {},
					alpha: {}
				}, this.blendState.color.srcFactor = "one", this.blendState.color.dstFactor = "zero", this.blendState.color.operation = "add", this.blendState.alpha.srcFactor = "one", this.blendState.alpha.dstFactor = "zero", this.blendState.alpha.operation = "add";
				break;
			case "disable":
	...
	switch (n) {
		case "none":
			...
			break;
		case "frontCCW":
			t.frontFace = "ccw", t.cullMode = "front";
			break;
		case "frontCW":
			t.frontFace = "cw", t.cullMode = "front";
			break;
		case "backCCW":
			t.frontFace = "ccw", t.cullMode = "back";
			break;
		case "backCW":
	...
	let a = (e, t) => {
		e.forEach((e) => {
			t.has(e.binding) ? t.get(e.binding).visibility |= e.visibility : t.set(e.binding, e);
		});
	};
	for (let o = 0; o < e.context.getLimits().maxBindGroups; o++) {
		let s = /* @__PURE__ */ new Map();
		if (r.has(o) && a(r.get(o), s), i.has(o) && a(i.get(o), s), s.size && (t.set(o, s.values()), t.size != o + 1)) {
			console.log(`[E][parseRenderBindGroupLayout] ${e.debugLabel} binding group should use in order from start [0 to ${n}], please check shader binding group index.`), t.clear();
			return;
		}
	}
	On({
		...
		bindGroupLayouts: e.bindGroupLayouts,
	...
	switch (t) {
		case "drawCount": return (t) => {
			let n = e.dispatch.getMaxDrawCount(), r = e.dispatch.getInstanceCount();
			t.draw(n, r);
		};
		case "drawIndexed": return (t) => {
			let n = e.dispatch.getIndexBuffer(), r = e.dispatch.getInstanceCount();
			t.setIndexBuffer(n.getGpuBuffer(null, "frameBegin"), n.getIndexFormat()), t.drawIndexed(n.getMaxDrawCount(), r, 0, 0, 0);
		};
		case "drawIndexedStorage": return (t) => {
			let n = e.dispatch.getIndexStorageBuffer(), r = e.dispatch.getInstanceCount();
			t.setIndexBuffer(n.getGpuBuffer(null, "frameBegin"), n.getIndexedFormat()), t.drawIndexed(n.getMaxDrawCount(), r, 0, 0, 0);
		};
		case "drawIndirect": return (t) => {
			let n = e.dispatch.getIndirectBuffer();
		...
		case "drawIndexedIndirect": return (t) => {
			let n = e.dispatch.getIndexStorageBuffer(), r = e.dispatch.getIndexedIndirectBuffer();
	...
	constructor(e, t = "none") {
		this.propertyName = e, this.propertyFormat = t;
	}
	getPropertyFormat = () => this.propertyFormat;
	...
	let t = (t, n) => {
		if (!e.bufferUniformRecordsMap.has(t)) {
			let n = /* @__PURE__ */ new Map();
			e.bufferUniformRecordsMap.set(t, n);
		}
		e.bufferUniformRecordsMap.get(t)?.set(n.name, n);
	}, n = [], r = [];
	return (e.uniforms?.getPropertyMap())?.forEach((i, a) => {
		let o = i.getPropertyFormat();
		switch (o) {
			case "uniformBuffer": {
				let r = i.getUniformBufferID(), s = {
					name: a,
					type: o,
					resourceID: r
				};
				t(r, s), e.uniformRecordMap.set(a, s), n.push(r);
				break;
			}
			case "storageBuffer": {
				let r = i.getStorageBufferID(), s = {
					name: a,
					type: o,
					resourceID: r
				};
			...
			case "textureStorage2D": {
				...
				t(n, s), e.uniformRecordMap.set(a, s), r.push(n);
				break;
			}
			case "textureSampler": {
				let n = i.getTextureSamplerID(), r = {
					type: o,
					name: a,
					resourceID: n
				};
				t(n, r), e.uniformRecordMap.set(a, r);
				break;
			}
			default: throw Error(`[E][parseUniform][holder][name] ${e.debugLabel}, unsupported property type: ${o}`);
		}
	}), n.length || r.length ? (e, t, i, a) => {
		n.forEach((n) => {
			i.getBuffer(n)?.getGpuBuffer(t, e);
		}), r.forEach((n) => {
	...
	(() => {
		...
		for (let e = 0; e < t; e++) {
			...
			if (t.length) {
				let r = Array.from(new Map(t.map((e) => [e.name, e])).values());
				n.set(e, r);
			}
		}
	})(), n.forEach((n, r) => {
		...
		if (n.forEach((t) => {
			let r = 0, a = t.name;
			if (!e.uniformRecordMap.has(a)) throw Error(`[E][emitUniforms] ${e.debugLabel} uniforms ${a} not exists.`);
			let o = e.uniformRecordMap.get(a);
			...
			switch (s) {
				...
				case x.Uniform: {
					...
					let s = {
						buffer: a.getGpuBuffer(null, "frameBegin"),
						offset: r,
						size: a.getByteLength()
					}, c = {
						binding: t.binding,
						resource: s
					};
				...
				default: throw Error(`[E][emitUniforms] ${e.debugLabel} missing uniforms, resourceBindings: ${n}`);
			}
		}), e.gourpIDWithBindGroupLayoutDescriptorMap.get(r).entries.length !== i.length) throw Error(`[E][emitUniforms] ${e.debugLabel} analysis bind_group_entries error.`);
		let a = {
			layout: e.gourpIDWithBindGroupLayoutMap.get(r),
			entries: i
	...
	samplerBindingType;
	constructor(e) {
		this.id = e.id, this.context = e.context, this.addressModeU = e.addressModeU || "clamp-to-edge", this.addressModeV = e.addressModeV || "clamp-to-edge", this.addressModeW = e.addressModeW || "clamp-to-edge", this.magFilter = e.magFilter || "nearest", this.minFilter = e.minFilter || "nearest", this.mipmapFilter = e.mipmapFilter || "nearest", this.lodMinClamp = e.lodMinClamp || 0, this.lodMaxClamp = e.lodMaxClamp || 1, this.anisotropy = e.anisotropy || 1, this.compareFunction = e.compareFunction, this.samplerBindingType = e.samplerBindingType || "filtering";
	}
	getID = () => this.id;
	...
	getSampler = (t) => {
		if (!e.SAMPLER_SET.has(t)) throw Error(`[E][SamplerState][getSampler] find sampler failed, id: ${t}`);
	...
	createTextureSampler = (n) => {
		let r = t(), i = new Ln({
			...
			addressModeV: n.addressModeV,
			addressModeW: n.addressModeW,
			magFilter: n.magFilter,
			minFilter: n.minFilter,
			mipmapFilter: n.mipmapFilter,
			lodMinClamp: n.lodMinClamp,
			lodMaxClamp: n.lodMaxClamp,
			anisotropy: n.anisotropy,
			samplerBindingType: n.samplerBindingType
		});
	...
	propertyFormat;
	constructor(e) {
		this.id = e.id, this.context = e.context, this.propertyFormat = e.propertyFormat;
	}
	getID = () => this.id;
	getPropertyFormat = () => this.propertyFormat;
}, Vn = class extends Bn {
	...
	constructor(e) {
		...
		}), this.computePipelineDescriptor = e.computePipelineDescriptor;
	}
	...
	constructor(e) {
		...
		}), this.renderPipelineDescriptor = e.renderPipelineDescriptor;
	}
	...
	getPipeline = (t) => {
		if (e.PIPELINE_SET.has(t)) return e.PIPELINE_SET.get(t);
	...
	constructor(e) {
		super({
			id: e.id,
			context: e.context
		}), this.texture = e.texture, this.depthLoadStoreFormat = e.depthLoadStoreFormat || "clearStore", this.depthCompareFunction = e.depthCompareFunction || "less-equal", this.stencilStateFormat = e.stencilFunctionFormat || "alwaysKeep", this.stencilLoadStoreFormat = e.stencilLoadStoreFormat || "loadStore", this.depthReadOnly = e.depthReadOnly || !1, this.depthClearValue = e.depthClearValue === void 0 ? 1 : e.depthClearValue, this.stencilReadOnly = e.stencilReadOnly || !1, this.stencilClearValue = e.stencilClearValue === void 0 ? 1 : e.stencilClearValue, this.depthBias = e.depthBias === void 0 ? void 0 : e.depthBias, this.depthBiasSlopeScale = e.depthBiasSlopeScale === void 0 ? void 0 : e.depthBiasSlopeScale;
	}
	updateState = () => {
		switch (this.depthStencilState = this.depthStencilState || { format: this.texture.getTextureFormat() }, this.depthCompareFunction) {
			case "never":
				this.depthStencilState.depthWriteEnabled = !1, this.depthStencilState.depthCompare = this.depthCompareFunction;
				break;
			case "equal":
			case "greater":
			case "greater-equal":
	...
	updateAttachment = () => {
		if (this.texture.useAsRenderAttachment(), this.depthStencilAttachment = { view: this.texture.getGpuTextureView() }, this.texture.isDetphTexture()) switch (this.depthLoadStoreFormat) {
			case "clearStore":
				this.depthStencilAttachment.depthClearValue = this.depthClearValue, this.depthStencilAttachment.depthLoadOp = "clear", this.depthStencilAttachment.depthStoreOp = "store", this.depthStencilAttachment.depthReadOnly = this.depthReadOnly;
				break;
			case "loadStore":
				this.depthStencilAttachment.depthClearValue = this.depthClearValue, this.depthStencilAttachment.depthLoadOp = "load", this.depthStencilAttachment.depthStoreOp = "store", this.depthStencilAttachment.depthReadOnly = this.depthReadOnly;
				break;
			default: throw Error(`[E][DepthStencilAttachment][updateRenderPassDepthStencilAttachment] unsupported depthLoadStoreFormat: ${this.depthLoadStoreFormat}`);
		}
		if (this.texture.isStencilTexture()) switch (this.stencilLoadStoreFormat) {
			case "clearStore":
				this.depthStencilAttachment.stencilClearValue = this.stencilClearValue, this.depthStencilAttachment.stencilLoadOp = "clear", this.depthStencilAttachment.stencilStoreOp = "store", this.depthStencilAttachment.stencilReadOnly = this.stencilReadOnly;
				break;
			case "loadStore":
				this.depthStencilAttachment.stencilClearValue = this.stencilClearValue, this.depthStencilAttachment.stencilLoadOp = "load", this.depthStencilAttachment.stencilStoreOp = "discard", this.depthStencilAttachment.stencilReadOnly = this.stencilReadOnly;
				break;
			default: throw Error(`[E][DepthStencilAttachment][updateRenderPassDepthStencilAttachment] unsupported stencilLoadStoreFormat: ${this.stencilLoadStoreFormat}`);
		}
	};
	...
	getAttachment = (t) => {
		if (!e.ATTACHMENT_SET.has(t)) throw Error(`[E]][AttachmentState][getAttachment] find attachment id: ${t} failed.`);
		return e.ATTACHMENT_SET.get(t);
	};
	createColorAttachment = (n) => {
		...
		if (!e.ATTACHMENT_SET.has(r)) {
			let t = new Sn({
				...
				context: this.context,
				texture: n.texture,
				blendFormat: n.blendFormat,
				colorLoadStoreFormat: n.colorLoadStoreFormat,
				clearColor: n.clearColor
			});
			e.ATTACHMENT_SET.set(r, t);
		}
	...
	createDepthStencilAttachment = (n) => {
		...
		if (!e.ATTACHMENT_SET.has(r)) {
			let t = new Wn({
				...
				context: this.context,
				texture: n.texture,
				depthBias: n.depthBias,
				depthBiasSlopeScale: n.depthBiasSlopeScale,
				depthLoadStoreFormat: n.depthLoadStoreFormat,
				depthCompareFunction: n.depthCompareFunction,
				stencilFunctionFormat: n.stencilFunctionFormat,
				stencilLoadStoreFormat: n.stencilLoadStoreFormat,
				depthReadOnly: n.depthReadOnly,
				depthClearValue: n.depthClearValue,
				stencilReadOnly: n.stencilReadOnly,
				stencilClearValue: n.stencilClearValue
			});
			e.ATTACHMENT_SET.set(r, t);
		}
		return e.ATTACHMENT_SET.get(r);
	...
	let t = e.dispatch.getPropertyFormat();
	switch (t) {
		case "computeDispatch": return (t) => {
			t.dispatchWorkgroups(e.dispatch.getGroupX(), e.dispatch.getGroupY(), e.dispatch.getGroupZ());
		};
	...
	createVertexBuffer = (e) => this.bufferState.createVertexBuffer({
		label: e.debugLabel || "vertexbuffer",
		totalByteLength: e.totalByteLength,
		rawData: e.rawData,
		handler: e.handler
	});
	createIndexBuffer = (e) => {
		if (e.rawData.byteLength % 4 != 0) throw Error(`[E][Compiler][createIndexBuffer] buffer bytelength must align with 4. current index buffer byte length: ${e.rawData.byteLength}`);
		return this.bufferState.createIndexBuffer({
			label: e.debugLabel || "indexbuffer",
			rawData: e.rawData
		});
	};
	createUniformBuffer = (e) => {
		if (e.totalByteLength >= this.context.getLimits().maxUniformBufferBindingSize) throw Error(`[E][createUniformBuffer] ${e.debugLabel} total byte length vaild.`);
		return this.bufferState.createUniformBuffer({
			label: e.debugLabel || "uniformbuffer",
			totalByteLength: e.totalByteLength,
			rawData: e.rawData,
			handler: e.handler
		});
	...
	createStorageBuffer = (e) => {
		if (e.totalByteLength >= this.context.getLimits().maxStorageBufferBindingSize) throw Error(`[E][createStorageBuffer] ${e.debugLabel} total byte length vaild.`);
		return this.bufferState.createStorageBuffer({
			label: e.debugLabel || "storagebuffer",
			totalByteLength: e.totalByteLength,
			bufferUsageFlags: e.bufferUsageFlags,
			rawDataArray: e.rawDataArray,
			handler: e.handler
	...
	createIndexedStorageBuffer = (e) => {
		if (e.totalByteLength >= this.context.getLimits().maxStorageBufferBindingSize) throw Error(`[E][createIndexedStorageBuffer] ${e.debugLabel} total byte length vaild.`);
		return this.bufferState.createIndexedStorageBuffer({
			label: e.debugLabel || "indexedstoragebuffer",
			totalByteLength: e.totalByteLength,
			rawDataArray: e.rawDataArray,
			handler: e.handler
		});
	};
	createIndirectBuffer = (e) => {
		if (e.totalByteLength >= this.context.getLimits().maxStorageBufferBindingSize) throw Error(`[E][createIndirectBuffer] ${e.debugLabel} total byte length vaild.`);
		return this.bufferState.createIndirectBuffer({
			label: e.debugLabel || "indirectbuffer",
			totalByteLength: e.totalByteLength,
			rawDataArray: e.rawDataArray,
			handler: e.handler
		});
	};
	createIndexedIndirectBuffer = (e) => {
		if (e.totalByteLength >= this.context.getLimits().maxStorageBufferBindingSize) throw Error(`[E][createIndexedIndirectBuffer] ${e.debugLabel} total byte length vaild.`);
		return this.bufferState.createIndexedIndirectBuffer({
			label: e.debugLabel || "indexedindirectbuffer",
			totalByteLength: e.totalByteLength,
			rawDataArray: e.rawDataArray,
			handler: e.handler
		});
	...
	createMapBuffer = (e) => {
		if (e.totalByteLength >= this.context.getLimits().maxStorageBufferBindingSize) throw Error(`[E][createMapBuffer] ${e.debugLabel} total byte length vaild.`);
		return this.bufferState.createMapBuffer({
			label: e.debugLabel || "mapbuffer",
			totalByteLength: e.totalByteLength,
			appendixBufferUsageFlags: e.appendixBufferUsageFlags,
			rawDataArray: e.rawDataArray,
			handler: e.handler
		});
	};
	createVertexShader = (e) => this.shaderState.createVertexShader({
		code: e.code,
		entryPoint: e.entryPoint
	});
	createFragmentShader = (e) => this.shaderState.createFragmentShader({
		code: e.code,
		entryPoint: e.entryPoint
	});
	createComputeShader = (e) => this.shaderState.createComputeShader({
		code: e.code,
		entryPoint: e.entryPoint
	});
	createColorAttachment = (e) => this.attachmentState.createColorAttachment({
		texture: e.texture,
		blendFormat: e.blendFormat,
		colorLoadStoreFormat: e.colorLoadStoreFormat,
		clearColor: e.clearColor
	});
	createDepthStencilAttachment = (e) => this.attachmentState.createDepthStencilAttachment({
		texture: e.texture,
		depthBias: e.depthBias,
		depthBiasSlopeScale: e.depthBiasSlopeScale,
		depthLoadStoreFormat: e.depthLoadStoreFormat,
		depthCompareFunction: e.depthCompareFunction,
		stencilFunctionFormat: e.stencilFunctionFormat,
		stencilLoadStoreFormat: e.stencilLoadStoreFormat,
		depthReadOnly: e.depthReadOnly,
		depthClearValue: e.depthClearValue,
		stencilReadOnly: e.stencilReadOnly,
		stencilClearValue: e.stencilClearValue
	});
	createSurfaceTexture2D = () => this.textureState.createSurfaceTexture2D();
	createTexture2D = (e) => this.textureState.createTexutre2D({
		width: e.width,
		height: e.height,
	...
	createTexture2DArray = (e) => this.textureState.createTexture2DArray({
		...
		height: e.height,
		depthOrArrayLayers: e.depthOrArrayLayers,
		textureDataArray: e.textureDataArray,
		handler: e.handler,
		textureFormat: e.textureFormat,
		mipmapCount: e.mipmapCount,
		appendixTextureUsages: e.appendixTextureUsages
	});
	createTextureCube = (e) => this.textureState.createTextureCube({
		width: e.width,
		height: e.height,
		faces: e.faces,
		textureFormat: e.textureFormat,
	...
	createTextureSampler = (e) => this.samplerState.createTextureSampler({
		...
		addressModeW: e.addressModeW,
		magFilter: e.magFilter,
		minFilter: e.minFilter,
		mipmapFilter: e.mipmapFilter,
		lodMinClamp: e.lodMinClamp,
		lodMaxClamp: e.lodMaxClamp,
		anisotropy: e.anisotropy,
		samplerBindingType: e.samplerBindingType
	});
	createComparisonSampler = (e) => this.samplerState.createComparisonSampler({
		addressModeU: e.addressModeU,
		addressModeV: e.addressModeV,
		addressModeW: e.addressModeW,
		magFilter: e.magFilter,
	...
	hanlderZ;
	constructor(e, t, n) {
		super("compute property", "computeDispatch"), typeof e == "number" ? this.groupX = e : typeof e == "function" && e.length === 0 && (this.hanlderX = e), typeof t == "number" ? this.groupY = t : typeof t == "function" && t.length === 0 && (this.hanlderY = t), typeof n == "number" ? this.groupZ = n : typeof n == "function" && n.length === 0 && (this.hanlderZ = n);
	}
	...
	getInstanceCount() {
		if (this.instanceCount) return this.instanceCount;
		throw Error("[E][RenderProperty][getInstanceCount] indvalid instance count.");
	}
	getIndexBuffer() {
		if (this.indexBuffer) return this.indexBuffer;
		throw Error("[E][RenderProperty][getIndexBufferID] missing indexBuffer in 'RenderProperty'.");
	}
	getIndexFormat() {
		if (this.indexBuffer) return this.indexBuffer.getIndexFormat();
		throw Error("[E][RenderProperty][getIndexFormat] missing indexBuffer in 'RenderProperty'.");
	}
	getIndexedIndirectBuffer = () => {
		if (this.indexedIndirectBuffer) return this.indexedIndirectBuffer;
		throw Error("[E][RenderProperty][getIndexedIndirectBuffer] invalid indexed indirect buffer.");
	};
	getIndexStorageBuffer = () => {
		if (this.indexedStorageBuffer) return this.indexedStorageBuffer;
		throw Error("[E][RenderProperty][getIndexStorageBuffer] invalid index storage buffer.");
	};
	getIndirectCountBuffer = () => {
		if (this.indirectDrawCountBuffer) return this.indirectDrawCountBuffer;
		throw Error("[E][RenderProperty][getIndirectCountBuffer] invalid indirect draw count buffer for multiDrawIndirect.");
	};
	getIndirectBuffer = () => {
		if (this.indirectBuffer) return this.indirectBuffer;
		throw Error("[E][RenderProperty][getIndirectCountBuffer] invalid indirect buffer.");
	};
};
//#endregion
